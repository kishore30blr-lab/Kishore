...
def load_data():
    ...
    try:
        conn = psycopg2.connect(
            dbname=os.environ.get("DB_NAME", "etl_testing"),
...
def create_slicing_charts(df):
    ...
    # Show table for Mar-Dec 2019 and 2020
    st.dataframe(df_slice2[['date', 'year', 'total_nonfarm']].rename(
        columns={'total_nonfarm': 'Employment'}
    ))

    # Show average for each year in the selected range
    yearly_avg = (
        df[(df['date'].dt.year >= year_range[0]) & (df['date'].dt.year <= year_range[1])]
        .groupby(df['date'].dt.year)['total_nonfarm']
        .mean()
        .reset_index()
        .rename(columns={'date': 'year', 'total_nonfarm': 'avg_total_nonfarm'})
    )
    st.dataframe(yearly_avg.rename(columns={'date': 'Year', 'avg_total_nonfarm': 'Average Employment'}))
...
def create_dicing_charts(df):
    ...
    if not significant_drops.empty:
        st.subheader("Recovery Time After >2% Employment Drops")
        st.write("For each month with a >2% drop, see how many months it took to return to the prior peak employment level:")
    ...
    if quarter == 'Q4':
        st.subheader("Q4: Month with Highest Payroll Growth Each Year")
        q4_df = df_quarter_interval.copy()
        # For each year, find the month with the highest MoM % change
        idx = q4_df.groupby('year')['pct_change_mom'].idxmax()
        q4_max = q4_df.loc[idx, ['year', 'month', 'pct_change_mom']].sort_values('year')
        q4_max = q4_max.rename(columns={'month': 'Month', 'pct_change_mom': 'Max MoM % Growth'})
        st.dataframe(q4_max)
...
def create_drill_down_charts(df):
    ...
    # Show table of monthly contributions (MoM change)
    highest_gain_df['mom_change'] = highest_gain_df['total_nonfarm'].diff()
    monthly_contrib = highest_gain_df[['month', 'total_nonfarm', 'mom_change']].copy()
    monthly_contrib = monthly_contrib.iloc[1:]  # skip first NaN
    monthly_contrib = monthly_contrib.sort_values('mom_change', ascending=False)
    st.dataframe(monthly_contrib.rename(columns={
        'month': 'Month',
        'total_nonfarm': 'Employment',
